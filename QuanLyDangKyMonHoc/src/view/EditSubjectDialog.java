...
public class EditSubjectDialog extends javax.swing.JDialog implements ActionListener {
    ...
    public EditSubjectDialog(java.awt.Frame parent, boolean modal, Subject oldSubject) {
        this(parent, modal);
    ...
    private void editSubject() {
        if (new Database().removeSubject(oldSubject) != -1) {}
        ...
        if (!id.isEmpty() && !name.isEmpty() && !numOfLessonString.isEmpty()) {
            newSubject.setId(id);
            newSubject.setName(name);
            newSubject.setNumOfLesson(Integer.parseInt(numOfLessonString));
            newSubject.setKind(type);
            homeFrm.editSubjectCallback(newSubject);

            if (new Database().addSubject(newSubject) != -1) {
    ...
    private void showData() {
        txtSubjectId.setText(oldSubject.getId());
        txtSubjectName.setText(oldSubject.getName());
        txtNumOfLesson.setText(oldSubject.getNumOfLesson() + "");
        for (int i = 0; i < comboSubjectType.getItemCount(); i++) {
            if (oldSubject.getKind().compareTo(
